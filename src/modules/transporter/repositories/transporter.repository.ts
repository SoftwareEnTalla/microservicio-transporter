...
import { logger } from '@core/logs/logger';
  ...
  export class TransporterRepository {
    ...
    async findAll(options?: FindManyOptions<Transporter>): Promise<Transporter[]> {
      logger.info('Ready to findAll Transporter on repository:',options);
    ...
    async findById(id: string): Promise<Transporter | null> {
      const tmp: FindOptionsWhere<Transporter> = { id } as FindOptionsWhere<Transporter>;
      logger.info('Ready to findById Transporter on repository:',tmp);
    ...
    async findByField(
      ...
      logger.info('Ready to findByField Transporter on repository:',options);
    ...
    async count(): Promise<number> {
      logger.info('Ready to count Transporter on repository...');
      let result= this.repository.count();
      logger.info('Was counted  instances of Transporter on repository:');
      return result;
    }
    ...
    async findAndCount(where?: Record<string, any>): Promise<[Transporter[], number]> {
      logger.info('Ready to findByField Transporter on repository:',where);
    ...
    async findOne(options?: Record<string, any>): Promise<Transporter | null> {
      if (!options || Object.keys(options).length === 0) {
        logger.warn('No conditions provided for finding Transporter.');
        return null;
      }
      ...
      if (isFindOneOptions) {
        logger.info('Ready to findOne (FindOneOptions) Transporter:', options);
        return this.repository.findOne(options as any);
      }
      const tmp: FindOptionsWhere<Transporter> = options as FindOptionsWhere<Transporter>;
      logger.info('Ready to findOneBy Transporter on repository:', tmp);
      return this.repository.findOneBy(tmp);
    }
    ...
    async findOneOrFail(where?: Record<string, any>): Promise<Transporter> {
      logger.info('Ready to findOneOrFail Transporter on repository:',where);
    ...
    async update(
        ...
        logger.info('Updated instance of Transporter with id:  was finded on repository:', instance);
